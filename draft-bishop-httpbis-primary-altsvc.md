---
title: The "Primary" Alt-Svc Parameter
abbrev: Primary Alt-Svc
...
Section 3, HTTP Co-Tenancy Fronting, but notes both discoverability and security
issues with that approach. This document provides a mechanism to address both
limitations.
...
# The "primary" Alt-Svc Extension {#extension}

When an origin wishes to nominate a "fronting server", it includes the `primary`
...
in the `primary` parameter in the SNI extension during the TLS handshake.  The
client MAY validate this certificate for authenticity according to {{!RFC2818}},
but MUST NOT make requests to the origin corresponding to this certificate if
the certificate has not been successfully validated.
...
servers publishing records with the `primary` parameter cannot be assured that
